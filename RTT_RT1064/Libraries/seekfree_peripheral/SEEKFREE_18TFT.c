...
void lcd_init(void)
{
    ...
    gpio_init(BL_PIN, GPO, 0, GPIO_PIN_CONFIG);
...
////////////////////////////////////////////////////////////////
//补充一些常用的TFT函数
//优先使用lcd_drawline_v和lcd_drawline_h
//lcd_drawLine效率较低

//画实心矩形
// void lcd_Fillctangle(uint16 x1, uint16 y1, uint16 x2, uint16 y2, uint16 color)
// {
...
void lcd_drawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16 color)
{
...
//画竖线
void lcd_drawline_v(uint16 x, uint16 y, uint16 height, uint16 color)
{
    lcd_set_region(x, y, x, y + height - 1);
...
//画横线
void lcd_drawline_h(uint16 x, uint16 y, uint16 width, uint16 color)
{
    lcd_set_region(x, y, x + width - 1, y);
